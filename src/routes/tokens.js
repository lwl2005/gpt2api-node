...
router.get('/', (req, res) => {
  try {
    ...
    const normalizedStatus = String(req.query.status || 'all').toLowerCase();
    const status = ['all', 'active', 'disabled', 'cooling', 'healthy', 'unhealthy', 'auto-disabled'].includes(normalizedStatus)
      ? normalizedStatus
      : 'all';

    const total = Token.getTotalCount({ keyword, status });
    const tokens = Token.getPaginated(offset, limit, { keyword, status }).map(maskSensitiveToken);
